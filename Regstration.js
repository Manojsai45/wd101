...
describe("", () => {
  ...
  it("The registration form should not accept invalid email addresses", () => {
    ...
    cy.get("#email").type("admin2@example.com"); // Fix email address
    cy.get("[type='submit']").click();
    cy.get("table").find("tr").contains("admin2@example.com").should("not.exist");
  ...
  it("The registration form should only accept users between 18 and 55 years old", () => {
    ...
    cy.get("#dob").click().type("2010-02-02");
    cy.get("[type='submit']").click();
